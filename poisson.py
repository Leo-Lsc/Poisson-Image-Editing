...
# Find the indicies of omega(where the mask is 1)
def omega_indicies(mask):
    nonzero = np.nonzero(mask)
    return zip(nonzero[0], nonzero[1])

# Determine whether a given index is on omega or not
...
def edge(index, mask):
    ...
    for pt in get_surrounding(index):
        # If the point is on omega, and a surrounding point isn't,
        # then it must be on an edge
...
def process(source, target, mask):
    ...
    indicies = list(omega_indicies(mask))
    N = len(indicies)
